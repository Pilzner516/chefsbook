...
- Complete redesign of shopping list print stylesheet
- Single column layout for tearable department sections
- Print checkboxes (□) on every row for hand-ticking
- Department headers: brand red (#C0392B), ALL CAPS, with item counts
- Full ingredient names (no truncation)
- Expanded unit abbreviations: tablespoons not T, teaspoons not tsp, etc.
- Print header with ChefsBook logo, list name, date printed
- Print footer with branding "Generated by ChefsBook — chefsbk.app"
- Recipe source shown on right of each row
- Alternating row backgrounds (#FAFAF8 / white) for readability
- Hide screen-only elements when printing (sidebar, buttons, controls)
- Removed old two-column layout (`print-two-col` disabled)

**Files Modified:**
- `apps/web/app/globals.css` - comprehensive shopping list print styles
- `apps/web/app/dashboard/shop/page.tsx` - print elements + expandUnit helper

**Deployed:** RPi5 via deploy-staging.sh, PM2 online

---

## 2026-04-29 (session BOOK-PREVIEW-1) TYPE: FEATURE

### Print Cookbook Fill Zones + Page Size Selector + Preview Scaling

**Fill Zones for Recipe Pages:**
- Added FillZone component to all 6 PDF templates (trattoria, studio, garden, heritage, nordic, bbq)
- Fill zone options: blank, chefs_notes (ruled lines), quote (styled quote block), decorative (ornament)
- Quote fill zone supports quoteText and quoteAttribution fields
- Each template uses its accent color for fill zone styling

**Page Size Selector:**
- Added page size selector in Book Settings panel
- Options: Letter (8.5×11), Trade (6×9), Large Trade (7×10), Digest (5.5×8.5), Square (8×8)
- Page size stored in BookLayout JSON

**Preview Scaling:**
- FlipbookPreview dynamically scales based on selected page size
- Maintains aspect ratio while filling available viewport
- Auto-rescales on window resize

**Types Added:**
- FillType: 'blank' | 'chefs_notes' | 'quote' | 'decorative' | 'custom'
- FillContent: { quoteText, quoteAttribution, customText, customImageUrl }
- PageSizeKey: 'letter' | 'trade' | 'large-trade' | 'digest' | 'square'

**Files Modified:**
- `apps/web/lib/book-layout.ts` - PageSizeKey, FillType, FillContent types
- `apps/web/lib/pdf-templates/types.ts` - export types + CookbookRecipe fields
- `apps/web/lib/pdf-templates/trattoria.tsx` - FillZone component + integration
- `apps/web/lib/pdf-templates/studio.tsx` - FillZone (dark theme adapted)
- `apps/web/lib/pdf-templates/garden.tsx` - FillZone (GREEN accent)
- `apps/web/lib/pdf-templates/heritage.tsx` - FillZone (SAGE accent)
- `apps/web/lib/pdf-templates/nordic.tsx` - FillZone (BLUE accent)
- `apps/web/lib/pdf-templates/bbq.tsx` - FillZone (AMBER accent)
- `apps/web/components/print/FlipbookPreview.tsx` - dynamic scaling + pageSize prop
- `apps/web/app/dashboard/print-cookbook/[id]/page.tsx` - reducer actions + UI
