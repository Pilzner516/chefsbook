...
| Recipe likes + like count | LIVE | /api/recipe/[id]/like (server), LikeButton.tsx (client), packages/db (toggleLike) | 30, 124, 128 | Server-side API route; plan gate (free=blocked+upgrade dialog); optimistic UI; notification via supabaseAdmin (no self-like, no unlike) |
