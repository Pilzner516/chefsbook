...
| FloatingTabBar (mobile) | LIVE | apps/mobile/components/FloatingTabBar.tsx, apps/mobile/app/_layout.tsx | 01, 203, P-SPEAK-FIX | **DO NOT MOVE**: Lives in ROOT `_layout.tsx` NOT in `(tabs)/_layout.tsx`. Exclusion list: index, auth/*, modal, messages, speak, cook-menu/*. Wizard screens with their own bottom nav must be added to exclusion list (not given paddingBottom workarounds). |
