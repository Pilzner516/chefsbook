...
| Instagram export import | LIVE | packages/ai (instagramExport.ts), apps/web/app/api/import/instagram-export/, apps/web/components/InstagramExportImporter.tsx | P-210 | Web only, Pro plan. ZIP parsed client-side (jszip). Haiku vision food classifier + caption extractor. User reviews grid before save. User's own photos used directly — no Replicate. New `instagramExport.ts` — do NOT confuse with deprecated `instagramImport.ts`. |
