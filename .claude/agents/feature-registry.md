...
| AI image generation | LIVE | apps/web/lib/imageGeneration.ts, /api/recipes/generate-image, /api/recipes/regenerate-image, packages/ai/src/imageThemes.ts | 147, 156, 181, 184B | Replicate Flux Dev ~$0.025/image (Pro) / Schnell ~$0.003 (others). Visible ChefsBook badge watermark (session 180). Creativity levels 1-5 from system_settings.image_creativity_level — levels 1-2 lead prompt with "closely resemble this source: <describeSourceImage output>" anchoring (session 181 fixed the full chain from import-time population to prompt-time threading). Levels 1-2 fall back to L3 with console.warn when source_image_description is NULL. Regeneration limit = 5 per recipe (session 184B raised from 1); regen_count is incremented (read-then-write), not hard-set. Pills keep showing until the 5th regen; modal shows "N regenerations remaining". |
