...
- **Extension import JSON.parse fails on complex recipes (session 200 diagnosed, NOT fixed):** callClaude() in packages/ai/src/client.ts:56 returns data.content[0].text without checking stop_reason. importFromUrl.ts:567 uses maxTokens:3000. On long multi-component recipes (e.g. foodandwine.com Bûche de Noël), Claude hits max_tokens, truncates mid-array, and extractJSON() (client.ts:67-71, no repair layer) throws a raw V8 error that leaks through /api/extension/import's 500 catch to popup.js:221 where it renders verbatim. Fix plan approved/deferred: (B) raise maxTokens to 6000 + surface stop_reason, (B.2) add jsonrepair fallback in extractJSON(), (C) popup never shows parser text + bump manifest 1.1.0→1.1.1, (D) logImportAttempt on extension-route catch. All 25 packages/ai/src/* call sites share the extractJSON brittleness.
