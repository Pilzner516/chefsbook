...
- **Mobile: speak.tsx:180 dead port-replacement hack** — `WEB_API_URL` computed as `(process.env.EXPO_PUBLIC_SUPABASE_URL ?? '...').replace(':8000', ':3000')`. With URL `https://api.chefsbk.app` there is no `:8000` to replace; result equals the Supabase URL — correct by coincidence (Cloudflare Tunnel routes `/api/*` to Next.js). Fix: add `EXPO_PUBLIC_WEB_URL=https://chefsbk.app` to `apps/mobile/.env.local` and use it directly.
