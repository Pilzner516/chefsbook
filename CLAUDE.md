...
- **Mobile: FloatingTabBar** — FIXED (session 203 + SPEAK-FLOATINGBAR-FIX + FLOATING-TABBAR-FIX 2026-05-05). Bar lives in root `_layout.tsx`, visible on all Stack screens. Wizard screens (`/speak`, `/cook-menu/*`, `/cook`) are excluded via `isWizard`. Tab onPress uses `router.navigate()` (not `push`) with clean URL paths (`/plan`, `/menus`, `/shop`) so tabs switch the active screen instead of stacking — fixed user-reported bug where Plan/Menus/Cart "would not load" when tapped from Stack screens. Index tab keeps `/(tabs)` to avoid landing-page redirect from `/`.
...
- **Mobile: splash safety net + auth init resilience (session MOBILE-SIGNIN-CRASH 2026-05-04)** — FIXED. Two new safeguards prevent stuck-splash hang when Supabase is unreachable: (1) `authStore.init()` wrapped in try/catch/finally so `loading: false` runs even when `getSession()` throws/retries; (2) `_layout.tsx` `SPLASH_MAX_MS=6000` safety-net `useEffect` dismisses overlay after 6s regardless of auth state. Fixes silent freeze where users couldn't reach landing page buttons because navigation guard stalled on `loading=true`.
- **Mobile: cook-menu/[id].tsx route conflict recurs** — File was deleted in session 204 + MOBILE-SIGNIN-CRASH but reappeared between sessions (not committed deletion → working-tree-only deletion lost). Permanent fix requires committing the deletion. If it reappears: `Get-Item -LiteralPath 'apps/mobile/app/cook-menu/[id].tsx' | Remove-Item -Force` (PowerShell brackets need `-LiteralPath`). The valid route is the directory `cook-menu/[id]/` with `index.tsx` + setup/active/briefing siblings.
- **Mobile: authStore minor follow-ups (architect-flagged 2026-05-04)** — `onAuthStateChange` subscription handle never stored/unsubscribed (subscription leak risk if `init()` ever called more than once); `loadProfile()` inside the listener not wrapped in try/catch (unhandled rejection on post-auth network failure). Both pre-existing, low priority, ~5 min each to fix.
- **Mobile: port-replacement hack FIXED (session MOBILE-IMAGE-GEN-FIX)** — `WEB_API_URL` now uses `process.env.EXPO_PUBLIC_WEB_URL ?? 'https://chefsbk.app'` instead of broken `.replace(':8000', ':3000')` pattern. Fixed in AiImageGenerationModal.tsx, speak.tsx, and cook/[id].tsx. `EXPO_PUBLIC_WEB_URL=https://chefsbk.app` added to mobile .env.local. Pending manual verification (see apps/mobile/TESTING-IMAGE-GEN.md).
- EXPO_PUBLIC_WEB_URL: mobile must use this env var for all calls to the Next.js web API (chefsbk.app:3000). NEVER derive the web URL from EXPO_PUBLIC_SUPABASE_URL or any port-replacement hack. The speak.tsx:180 port-replacement is dead code — do not replicate the pattern. Confirmed root cause of IMAGE-GEN-FIX bug (ecaf2f2).
