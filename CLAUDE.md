...
- **Mobile: FloatingTabBar** — FIXED (session 203 + SPEAK-FLOATINGBAR-FIX). Bar now lives in root `_layout.tsx` and is visible on all Stack screens. Wizard screens (`/speak`, `/cook-menu/*`) are excluded via the `isWizard` check — they have their own bottom navigation.
