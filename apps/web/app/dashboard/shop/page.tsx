      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {(() => {
            ...
            return sortedGroups.map(([storeName, groupLists]) => {
              const store = stores.find((s) => s.name === storeName) ?? null;
              return (
                <div key={storeName} className="bg-cb-card border border-cb-border rounded-card overflow-hidden">
                  {/* Store group header */}
