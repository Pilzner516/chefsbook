                  ...
                  return (
                    <Link key={recipe.id} href={`/recipe/${recipe.id}`} className="group">
                      <div className="bg-cb-card border border-cb-border rounded-card overflow-hidden hover:border-cb-primary/50 transition-colors">
                        <div className="h-32 overflow-hidden bg-cb-bg">
                          <img
                            src={imageUrl ? proxyIfNeeded(imageUrl) : CHEFS_HAT_URL}
                            alt={recipe.title}
                            className={`w-full h-full object-cover group-hover:scale-105 transition-transform ${!imageUrl ? 'p-6 opacity-30' : ''}`}
                          />
                        </div>
                        <div className="p-3">
                          <h3 className="font-semibold text-sm mb-1 group-hover:text-cb-primary transition-colors line-clamp-1">{recipe.title}</h3>
                          <div className="flex items-center gap-2 text-xs text-cb-secondary">
                            {recipe.cuisine && <span className="bg-cb-primary-soft text-cb-primary px-1.5 py-0.5 rounded text-[10px]">{recipe.cuisine}</span>}
              ...
              {techniques.map((tech) => (
                <Link key={tech.id} href={`/technique/${tech.id}`} className="group">
                  <div className="bg-cb-card border border-cb-border rounded-card overflow-hidden hover:border-cb-primary/50 transition-colors">
                    ...
                    <div className="p-3">
                      <h3 className="font-semibold text-sm mb-1 group-hover:text-cb-primary transition-colors">{tech.title}</h3>
                      {tech.difficulty && (
                        <span className="bg-cb-green-soft text-cb-green px-1.5 py-0.5 rounded text-[10px]">{tech.difficulty}</span>
                      )}
                      {tech.description && (
                        <p className="text-xs text-cb-secondary mt-1 line-clamp-2">{tech.description}</p>
                      )}
                    </div>
                  </div>
                </Link>
              ...
              {cookbooks.map((cb) => (
                <Link key={cb.id} href={`/dashboard/cookbooks/${cb.id}`} className="group">
                  <div className="bg-cb-card border border-cb-border rounded-card overflow-hidden hover:border-cb-primary/50 transition-colors">
                    ...
                    <div className="p-3">
                      <h3 className="font-semibold text-sm mb-1 group-hover:text-cb-primary transition-colors">{cb.title}</h3>
                      {cb.description && (
                        <p className="text-xs text-cb-secondary line-clamp-2">{cb.description}</p>
                      )}
                    </div>
                  </div>
                </Link>
