...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Sidebar */}
      <aside className="w-60 border-r border-cb-border bg-cb-card flex flex-col shrink-0">
        <div className="p-5 border-b border-cb-border">
        ...
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map((item, i) => {
            const active = pathname === item.href && i === 0
              ? pathname === '/dashboard'
              : pathname === item.href;
            return (
              <Link
                key={item.label}
                href={item.href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-input text-sm font-medium transition-colors ${
                  active
                    ? 'bg-cb-primary/10 text-cb-primary'
                    : 'text-cb-muted hover:text-cb-text hover:bg-cb-bg'
                }`}
              >
                {item.icon}
                {item.label}
