  ...
  return (
    <main className="min-h-screen bg-cb-bg text-cb-text">
      <nav className="flex items-center justify-between px-6 py-4 border-b border-cb-border">
        ...
        <Link href="/dashboard" className="bg-cb-primary text-cb-bg px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90">
          Try Chefsbook
        </Link>
      </nav>

      <article className="max-w-3xl mx-auto py-12 px-6">
        <div className="bg-cb-surface border border-cb-border rounded-xl p-4 mb-8 text-center text-sm text-cb-text-secondary">
          This recipe was shared with you via Chefsbook
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {recipe.cuisine && <span className="bg-cb-primary/10 text-cb-primary text-sm px-3 py-1 rounded-lg">{recipe.cuisine}</span>}
          {recipe.course && <span className="bg-cb-accent/10 text-cb-accent text-sm px-3 py-1 rounded-lg">{recipe.course}</span>}
          {recipe.total_minutes != null && recipe.total_minutes > 0 && (
            <span className="bg-cb-surface-alt text-cb-text-secondary text-sm px-3 py-1 rounded-lg">{formatDuration(recipe.total_minutes)}</span>
          )}
          <span className="bg-cb-surface-alt text-cb-text-secondary text-sm px-3 py-1 rounded-lg">{recipe.servings} servings</span>
        </div>
          ...
          <ul className="space-y-2">
            {recipe.ingredients.map((ing) => (
              <li key={ing.id} className="flex items-baseline gap-3">
                ...
                <span>{ing.ingredient}{ing.preparation && <span className="text-cb-text-secondary">, {ing.preparation}</span>}</span>
              </li>
            ))}
          </ul>
        </section>
            ...
            {recipe.steps.map((step) => (
              <li key={step.id} className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  {step.timer_minutes && <p className="text-cb-primary text-sm mt-1">{'\u23F1'} {step.timer_minutes} min</p>}
                </div>
              </li>
            ))}
          </ol>
