  ...
  return (
    <div>
      ...
      <div className="mb-4">
        {imageLoading ? (
          <div className="w-full h-48 bg-cb-bg rounded-card flex items-center justify-center animate-pulse">
            <span className="text-sm text-cb-secondary">Finding image...</span>
          </div>
        ) : recipeImageUrl ? (
          <div>
          ...
          <div className="flex gap-2">
            <button onClick={() => fetchImage(1)} className="flex-1 h-36 bg-cb-bg rounded-card border-2 border-dashed border-cb-border flex flex-col items-center justify-center hover:border-cb-primary transition-colors">
              <svg className="w-6 h-6 text-cb-secondary mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904 9 18.75l-.813-2.846a4.5 4.5 0 0 0-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 0 0 3.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 0 0 3.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 0 0-3.09 3.09Z" /></svg>
              <span className="text-xs text-cb-secondary">Generate image</span>
            </button>
            <label className="flex-1 h-36 bg-cb-bg rounded-card border-2 border-dashed border-cb-border flex flex-col items-center justify-center hover:border-cb-primary transition-colors cursor-pointer">
              ...
              <span className="text-xs text-cb-secondary">Upload photo</span>
      ...
      {cookbookAttribution && (
        <div className="bg-cb-bg border border-cb-border rounded-card p-3 mb-4 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {cookbookAttribution.author && <p className="text-[10px] text-cb-secondary">by {cookbookAttribution.author}</p>}
            {cookbookAttribution.pageNumber && <p className="text-[10px] text-cb-secondary">Page {cookbookAttribution.pageNumber}</p>}
      ...
      <div className="bg-cb-card border border-cb-border rounded-card p-6 mb-6">
        <input value={recipe.title ?? ''} onChange={(e) => setRecipe({ ...recipe, title: e.target.value })} className="text-2xl font-bold w-full bg-transparent outline-none mb-2 placeholder:text-cb-secondary" placeholder="Recipe title" />
        <textarea value={recipe.description ?? ''} onChange={(e) => setRecipe({ ...recipe, description: e.target.value })} rows={2} className="w-full bg-transparent outline-none text-cb-secondary text-sm leading-relaxed mb-4 resize-none" placeholder="Description" />

        <div className="flex flex-wrap gap-3 mb-6 text-sm">
          <label className="flex items-center gap-1.5"><span className="text-cb-secondary">Prep:</span><input type="number" value={recipe.prep_minutes ?? ''} onChange={(e) => setRecipe({ ...recipe, prep_minutes: parseInt(e.target.value) || null })} className="w-14 bg-cb-bg border border-cb-border rounded px-2 py-1 text-sm outline-none" /><span className="text-cb-secondary text-xs">min</span></label>
          <label className="flex items-center gap-1.5"><span className="text-cb-secondary">Cook:</span><input type="number" value={recipe.cook_minutes ?? ''} onChange={(e) => setRecipe({ ...recipe, cook_minutes: parseInt(e.target.value) || null })} className="w-14 bg-cb-bg border border-cb-border rounded px-2 py-1 text-sm outline-none" /><span className="text-cb-secondary text-xs">min</span></label>
          <label className="flex items-center gap-1.5"><span className="text-cb-secondary">Servings:</span><input type="number" value={recipe.servings ?? ''} onChange={(e) => setRecipe({ ...recipe, servings: parseInt(e.target.value) || null })} className="w-14 bg-cb-bg border border-cb-border rounded px-2 py-1 text-sm outline-none" /></label>
          <label className="flex items-center gap-1.5"><span className="text-cb-secondary">Cuisine:</span><input value={recipe.cuisine ?? ''} onChange={(e) => setRecipe({ ...recipe, cuisine: e.target.value })} className="w-24 bg-cb-bg border border-cb-border rounded px-2 py-1 text-sm outline-none" /></label>
        </div>

        <h3 className="text-sm font-bold text-cb-secondary uppercase tracking-wide mb-2">Ingredients</h3>
        <div className="space-y-1 mb-6">
          {(recipe.ingredients ?? []).map((ing, i) => (
            <div key={i} className="grid items-start gap-1 text-sm" style={{ gridTemplateColumns: '40px 80px 1fr 24px' }}>
              ...
              <span className="text-cb-secondary">{ing.unit ? abbreviateUnitMedium(ing.unit) : ''}</span>
              <span className="break-words">{ing.ingredient}</span>
              <button onClick={() => setRecipe({ ...recipe, ingredients: recipe.ingredients?.filter((_, j) => j !== i) })} className="text-cb-secondary hover:text-cb-primary text-xs text-center">✕</button>
            </div>
          ))}
        </div>

        <h3 className="text-sm font-bold text-cb-secondary uppercase tracking-wide mb-2">Steps</h3>
      ...
      <div className="flex gap-3">
        <button onClick={onBack} className="border border-cb-border px-5 py-3 rounded-input text-sm font-medium text-cb-secondary hover:text-cb-text">&larr; Back</button>
        {onRegenerate && <button onClick={onRegenerate} className="border border-cb-border px-5 py-3 rounded-input text-sm font-medium text-cb-secondary hover:text-cb-text">Regenerate</button>}
        <span className="flex-1" />
