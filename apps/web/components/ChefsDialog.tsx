...
export default function ChefsDialog({ open, icon, title, body, buttons, children, onClose }: ChefsDialogProps) {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      {/* Container */}
      <div
        className="relative bg-white rounded-2xl p-6 max-w-[440px] w-[calc(100%-32px)]"
        style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.12)' }}
        onClick={(e) => e.stopPropagation()}
      >
        {icon && <div className="text-[32px] mb-3 text-center">{icon}</div>}
        <h3 className="text-lg font-semibold text-[#1a1a1a] mb-2 text-center">{title}</h3>
        {body && (
          <div className="text-sm text-gray-500 leading-relaxed mb-6 text-center">
