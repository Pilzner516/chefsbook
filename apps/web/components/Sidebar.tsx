  ...
  return (
    <aside className={`h-screen sticky top-0 border-r border-cb-border bg-cb-card flex flex-col shrink-0 transition-all duration-200 overflow-y-auto ${collapsed ? 'w-14' : 'w-60'}`}>
      <div className="p-3 border-b border-cb-border flex items-center gap-2">
        <button onClick={toggleCollapse} className="text-cb-muted hover:text-cb-primary shrink-0 p-1.5 rounded-input hover:bg-cb-bg transition-colors" title={collapsed ? 'Show menu' : 'Hide menu'}>
      ...
      <nav className="flex-1 p-2 space-y-0.5">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.label} href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-input text-sm font-medium transition-colors ${active ? 'bg-cb-primary/10 text-cb-primary' : 'text-cb-muted hover:text-cb-text hover:bg-cb-bg'}`} title={collapsed ? item.label : undefined}>
              {item.icon}
              {!collapsed && (
                <>
                  <span className="flex-1">{item.label}</span>
                  {(item as any).pro && <span className="text-[8px] font-bold bg-amber-100 text-amber-700 px-1 py-0.5 rounded">PRO</span>}
                  {count != null && count > 0 && <span className="text-[10px] text-cb-muted">{count}</span>}
                </>
              )}
            </Link>
          );
      ...
      <div className="p-2 border-t border-cb-border space-y-1">
        <Link href="/dashboard/settings" className={`flex items-center gap-3 px-3 py-2.5 rounded-input text-sm font-medium transition-colors ${pathname === '/dashboard/settings' ? 'bg-cb-primary/10 text-cb-primary' : 'text-cb-muted hover:text-cb-text hover:bg-cb-bg'}`} title={collapsed ? 'Settings' : undefined}>
        ...
        {!collapsed && user && (
          <div className="flex items-center gap-2 px-3 py-1.5">
            ...
            <p className="text-xs text-cb-muted truncate flex-1">{user.email}</p>
          </div>
        )}
        <button onClick={async () => { await supabase.auth.signOut(); router.push('/auth'); }} className={`flex items-center gap-3 w-full px-3 py-2 rounded-input text-sm font-medium text-cb-muted hover:text-cb-text hover:bg-cb-bg transition-colors ${collapsed ? 'justify-center' : ''}`} title={collapsed ? 'Sign out' : undefined}>
