    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
      <div className="bg-cb-card w-full max-w-3xl max-h-[90vh] rounded-card shadow-xl flex flex-col mx-4">
        ...
        <div className="px-5 pt-4 flex gap-1 shrink-0">
          {['Days & Meals', 'Preferences', 'Sources', 'Review'].map((label, i) => (
            <div key={i} className="flex-1">
              ...
              <p className={`text-[10px] ${step === i + 1 ? 'text-cb-primary font-semibold' : 'text-cb-muted'}`}>{i + 1}. {label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {step === 1 && (
            <div>
              ...
              <div className="flex gap-1.5 mb-2">
                {DAYS.map((d, i) => (
                  <button key={d} onClick={() => toggleDay(d)} className={`flex-1 py-2 rounded-input text-xs font-medium ${selectedDays.includes(d) ? 'bg-cb-primary text-white' : 'bg-cb-bg text-cb-muted'}`}>{DAY_LABELS[i]}</button>
                ))}
              </div>
              ...
              <div className="flex gap-2 mb-4">
                {SLOTS.map((s) => (
                  <button key={s} onClick={() => toggleSlot(s)} className={`px-4 py-2 rounded-full text-xs font-medium ${selectedSlots.includes(s) ? 'bg-cb-primary text-white' : 'bg-cb-bg text-cb-muted'}`}>
                    {s === 'breakfast' ? '🌅' : s === 'lunch' ? '☀️' : s === 'dinner' ? '🌙' : '🍎'} {s.charAt(0).toUpperCase() + s.slice(1)}
                  </button>
                ))}
              </div>
              <label className="flex items-center gap-2 text-sm text-cb-muted">
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {DIETS.map((d) => (
                    <button key={d} onClick={() => toggleDiet(d)} className={`px-3 py-1.5 rounded-full text-xs font-medium ${dietary.includes(d) ? 'bg-cb-green text-white' : 'bg-cb-bg text-cb-muted'}`}>{d}</button>
                  ))}
                </div>
              </div>
              <div>
              ...
              <div>
                <h4 className="text-sm font-semibold mb-1">Cuisine variety <span className="font-normal text-cb-muted">{cuisineVariety < 30 ? '(same cuisine)' : cuisineVariety > 70 ? '(mix it up!)' : '(moderate)'}</span></h4>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2">
                  {CUISINES.map((c) => (
                    <button key={c} onClick={() => toggleCuisine(c)} className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${preferredCuisines.includes(c) ? 'bg-cb-primary text-white' : 'bg-cb-bg text-cb-muted'}`}>{c}</button>
                  ))}
                </div>
              </div>
              <div>
                <h4 className="text-sm font-semibold mb-1">Effort level <span className="font-normal text-cb-muted">{effortLevel < 30 ? '(quick & easy)' : effortLevel > 70 ? '(love to cook)' : '(moderate)'}</span></h4>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {SERVINGS.map((s) => (
                    <button key={s} onClick={() => setServings(s)} className={`w-12 h-10 rounded-input text-sm font-medium ${servings === s ? 'bg-cb-primary text-white' : 'bg-cb-bg text-cb-muted'}`}>{s}{s === 6 ? '+' : ''}</button>
                  ))}
                </div>
              </div>
            </div>
                ...
                <button key={opt.key} onClick={() => setSource(opt.key)} className={`w-full text-left p-4 rounded-card border-2 transition-colors ${source === opt.key ? 'border-cb-primary bg-cb-primary/5' : 'border-cb-border hover:border-cb-primary/30'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-cb-muted">{opt.desc}</p>
                    </div>
                  </div>
                </button>
              ))}
          ...
          {step === 4 && !generating && (
            <div>
              <p className="text-sm text-cb-muted mb-4">{plan.length} meals planned</p>
              <div className="space-y-2">
                {DAYS.filter((d) => selectedDays.includes(d)).map((day) => {
                  ...
                  return (
                    <div key={day}>
                      <p className="text-xs font-bold text-cb-muted uppercase mb-1">{day.charAt(0).toUpperCase() + day.slice(1)}</p>
                        ...
                        {daySlots.map((entry) => (
                          <div key={`${entry.day}-${entry.slot}`} className="flex items-center gap-3 bg-cb-bg rounded-input px-3 py-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-cb-muted">{entry.cuisine} · {entry.estimated_time}min · {entry.reason}</p>
                            </div>
                            <button onClick={() => swapSlot(day, entry.slot)} className="text-cb-muted hover:text-cb-primary shrink-0" title="Swap">
                            ...
                            <button onClick={() => removeSlot(day, entry.slot)} className="text-cb-muted hover:text-cb-primary shrink-0" title="Remove">
        ...
        <div className="p-5 border-t border-cb-border flex items-center gap-3 shrink-0">
          {step > 1 && step < 4 && <button onClick={() => setStep((s) => (s - 1) as any)} className="text-sm text-cb-muted hover:text-cb-text">&larr; Back</button>}
          ...
          {step === 4 && !generating && (
            <>
              <button onClick={() => setStep(2)} className="text-sm text-cb-muted hover:text-cb-text">&larr; Change preferences</button>
              <button onClick={generate} className="border border-cb-border px-4 py-2 rounded-input text-sm font-medium text-cb-muted hover:text-cb-text">Regenerate</button>
